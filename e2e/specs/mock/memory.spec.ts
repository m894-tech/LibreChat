 ...
 * the MEMORIES.USE permission and exposes the ephemeral memory capability (the
 * inline set_memory/delete_memory tools). This spec drives it end to end: enable
 * from Session → Tools, then confirm the toggle reaches the backend payload as
 * `ephemeralAgent.memory === true` on the next send.
 */
test.describe('memory badge', () => {
  test('toggles on from Session tools and is sent with the request', async ({ page }) => {
    test.setTimeout(120000);
    await page.goto('/c/new', { timeout: 10000 });

    // Mock Provider A is a custom endpoint, so the ephemeral tool grid is shown.
